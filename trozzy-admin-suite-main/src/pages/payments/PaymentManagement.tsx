...
const PaymentManagement = () => {
  ...
  useEffect(() => {
    const run = async () => {
      try {
        const [tx, methods, st] = await Promise.all([
          paymentAPI.getAllTransactions({ status: 'completed' }),
          paymentAPI.getPaymentMethods(),
          paymentAPI.getPaymentStats(),
        ]);

        if (tx?.success) setTransactions(Array.isArray(tx.data) ? tx.data : []);
        if (methods?.success) setPaymentMethods(Array.isArray(methods.data) ? methods.data : []);
  ...
  const filteredTransactions = transactions.filter((transaction) => {
    ...
    const matchesSearch =
      ...
      (transaction.upiId ? transaction.upiId.toLowerCase().includes(q) : false);
  ...
  const columns = [
    ...
    { key: 'amount', header: 'Amount', render: (t: PaymentTransaction) => `₹${t.amount.toFixed(2)}` },
    {
      key: 'paymentMethod',
      header: 'Method',
      render: (t: PaymentTransaction) => getMethodIcon(t.paymentMethod),
    },
    {
      key: 'status',
      header: 'Status',
    ...
    },
    {
      key: 'createdAt',
      header: 'Date',
      render: (t: PaymentTransaction) => new Date(t.createdAt).toLocaleDateString(),
    },
    {
      key: 'actions',
      header: 'Actions',
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">View payment transactions. Refunds are disabled.</p>
        </div>
        <Button
          onClick={async () => {
            try {
              const response = await paymentAPI.getAllTransactions({ status: 'completed' });
              if (response?.success) setTransactions(Array.isArray(response.data) ? response.data : []);
            } catch (e) {
