...
function mapProduct(req: Request, p: any) {
  ...
  return {
    ...
    shippingCharge: Number(p?.shippingCharge ?? p?.management?.shipping?.shippingCharge ?? 0) || 0,
...
async function upsertFromManagement(id: string, values: any) {
  ...
  const next = {
    ...
    shippingCharge: Number(values.shipping.shippingCharge ?? 0) || 0,
...
router.post("/draft", async (req: Request, res: Response) => {
  ...
  const doc = await ProductModel.create({
    ...
    badge: "",
    brand: "",
    shippingCharge: Number(draftValues.shipping.shippingCharge ?? 0) || 0,
...
router.post("/publish", async (req: Request, res: Response) => {
  ...
  const doc = await ProductModel.create({
    ...
    shippingCharge: Number(nextValues.shipping.shippingCharge ?? 0) || 0,
